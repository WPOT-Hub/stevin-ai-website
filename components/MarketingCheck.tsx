...
const DIAGNOSE: Record<string, Diagnose> = {
  ...
  CONSENT_WEIGEREN_VERSTOPT: {
    ...
    kop: 'Je banner maakt accepteren makkelijk en weigeren niet.',
    gevolg: 'Wie niet wil, klikt dan toch op ja. Je cijfers rusten op toestemming die niet echt gegeven is, en daar wordt tegenwoordig op gecontroleerd.',
    keten: ['Cookiebanner', 'Keuze', 'Meting', 'Aanvraag'], kapot: 1, woord: 'geen echte keuze',
    punten: [['Accepteren', 'Een klik'], ['Weigeren', 'Niet op dezelfde laag'], ['Gevolg', 'Toestemming zonder keuze']],
    herstel: 'Wij zetten weigeren naast accepteren, even groot, en zorgen dat de meting daarna precies doet wat de bezoeker koos. Je cijfers zijn dan schoon en verdedigbaar.',
    vraag: 'Weet jij wat er gebeurt als iemand op je site nee wil zeggen?',
    rood: 'Weigeren niet even makkelijk',
  },
  TECH_NO_VIEWPORT: {
    label: 'Aandacht voor je site op de telefoon',
    kop: 'Je site vertelt een telefoon niet hoe breed hij is.',
    gevolg: 'Op een telefoon verschijnt dan de desktopversie in het klein. Wie je vanaf de bank opzoekt moet knijpen en schuiven, en haakt af voordat hij je nummer vindt.',
    keten: ['Zoeken', 'Site op telefoon', 'Contact', 'Aanvraag'], kapot: 1, woord: 'onleesbaar op mobiel',
    punten: [['Viewport', 'Ontbreekt'], ['Telefoon', 'Toont desktopversie verkleind'], ['Gevolg', 'Bezoeker haakt af']],
    herstel: 'Dit is een regel in de kop van je site, voor wie hem bouwt. Wij meten daarna of het mobiele bezoek weer aanvragen oplevert.',
    vraag: 'Heb je je eigen site de laatste tijd op je telefoon geopend?',
    rood: 'Geen mobiele weergave ingesteld',
  },
  SEO_CANONICAL_OTHER_DOMAIN: {
    label: 'Aandacht voor je vindbaarheid',
    kop: 'Je homepage zegt tegen Google dat het origineel ergens anders staat.',
    gevolg: 'Google kan dan het andere adres tonen in plaats van het jouwe. Bezoek en aanvragen landen op een adres dat jij misschien niet bedoelde.',
    keten: ['Google', 'Canonical', 'Je website', 'Aanvraag'], kapot: 1, woord: 'verwijst weg',
    punten: [['Canonical', 'Naar een ander domein'], ['Je site', 'Landt op zichzelf'], ['Gevolg', 'Google kiest het andere adres']],
    herstel: 'Eerst nakijken of dit bedoeld is. Zo niet, dan is het een regel in de kop van je site, en wij meten daarna of je zoekverkeer terugkomt.',
    vraag: 'Is dat andere adres van jou, en wil je dat Google dat toont?',
    rood: 'Canonical naar ander domein',
  },
  MEASUREMENT_MULTIPLE_ANALYTICS: {
    label: 'Aandacht voor je cijfers',
    kop: 'Er tellen twee systemen tegelijk mee op je site.',
    gevolg: 'Die twee komen bijna nooit op hetzelfde uit. Dan is de vraag welk getal klopt, en dat kost je vertrouwen in allebei.',
    keten: ['Bezoeker', 'Meting', 'Rapport', 'Jij'], kapot: 2, woord: 'twee uitkomsten',
    punten: [['Meetpakketten', 'Twee naast elkaar'], ['Cijfers', 'Lopen uiteen'], ['Gevolg', 'Welk getal is waar?']],
    herstel: 'Wij bepalen met je welk pakket leidend is en zorgen dat het andere daar niet tegenin telt. Je stuurt daarna op een cijfer in plaats van twee.',
    vraag: 'Weet jij welk van de twee je aanhoudt als ze verschillen?',
    rood: 'Twee metingen naast elkaar',
  },
        ...
        <>
          ...
          {uitkomst.scorecard?.some((regel) => regel.status === 'niet_gezien') && (
            <section className="mt-8 border-t border-[var(--color-border)] pt-5" aria-labelledby="scan-onvoldoende-zicht">
              ...
              {/* Een keer de zin, met de gebieden ervoor. Per gebied dezelfde alinea
                  herhalen leest als een sjabloon, en dat ondermijnt precies het
                  vertrouwen dat deze sectie moet opbouwen. */}
              <p className="mt-3 text-[15px] leading-relaxed text-[var(--color-muted)]">
                Van buitenaf konden we{' '}
                <span className="font-semibold text-[var(--color-primary)]">
                  {uitkomst.scorecard
                    .filter((regel) => regel.status === 'niet_gezien')
                    .map((regel) => regel.label.toLowerCase())
                    .join(' en ')}
                </span>{' '}
                niet vaststellen. Dat betekent niet dat er iets mis is, maar ook niet dat het goed staat: met toegang
                tot je eigen accounts is het in een half uur na te kijken. Dat doen we graag vrijblijvend met je samen.
              </p>
            </section>
          )}
