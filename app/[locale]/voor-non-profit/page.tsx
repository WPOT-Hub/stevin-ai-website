...
const featureIcons = [
  <ShieldCheck key="1" className="w-5 h-5 text-accent" />,
  <Heart key="2" className="w-5 h-5 text-accent" />,
  <Mail key="3" className="w-5 h-5 text-accent" />,
  <GitMerge key="4" className="w-5 h-5 text-accent" />,
  <Newspaper key="5" className="w-5 h-5 text-accent" />,
  <CalendarClock key="6" className="w-5 h-5 text-accent" />,
]

export default async function VoorNonProfitPage({ params }: Props) {
  const { locale } = await params
  setRequestLocale(locale)
  const t = await getTranslations('voorNonProfit')
