...
import { Layers, Search, Radio, BarChart3, Inbox, MapPin } from 'lucide-react'

type Props = { params: Promise<{ locale: string }> }

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'fmcg' })
  return localizedMetadata({ path: '/fmcg', locale, title: `Stevin ${t('eyebrow')}, ${t('h1')}`, description: t('sub') })
}

const featureIcons = [
  <Layers key="1" className="w-5 h-5 text-accent" />,
  <Search key="2" className="w-5 h-5 text-accent" />,
  <Radio key="3" className="w-5 h-5 text-accent" />,
  <BarChart3 key="4" className="w-5 h-5 text-accent" />,
  <Inbox key="5" className="w-5 h-5 text-accent" />,
  <MapPin key="6" className="w-5 h-5 text-accent" />,
]

export default async function FmcgPage({ params }: Props) {
  const { locale } = await params
  setRequestLocale(locale)
  const t = await getTranslations('fmcg')
    ...
    <main>
      ...
      <section className="bg-primary -mt-[72px]" style={{ padding: 'calc(96px + 72px) 24px 128px' }}>
        <div className="mx-auto max-w-[1200px]">
          ...
          <p className="text-white/60 leading-[1.55]" style={{ fontSize: '20px', maxWidth: '600px', marginTop: '32px' }}>
            {t('sub')}
          </p>
