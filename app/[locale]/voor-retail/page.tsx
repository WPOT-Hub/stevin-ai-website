...
import { Footprints, Network, MapPin, CloudSun, Radar, Eye } from 'lucide-react'

type Props = { params: Promise<{ locale: string }> }

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'voorRetail' })
  return localizedMetadata({ path: '/voor-retail', locale, title: `Stevin ${t('eyebrow')}, ${t('h1')}`, description: t('sub') })
}

const featureIcons = [
  <Footprints key="1" className="w-5 h-5 text-accent" />,
  <Network key="2" className="w-5 h-5 text-accent" />,
  <MapPin key="3" className="w-5 h-5 text-accent" />,
  <Eye key="4" className="w-5 h-5 text-accent" />,
  <CloudSun key="5" className="w-5 h-5 text-accent" />,
  <Radar key="6" className="w-5 h-5 text-accent" />,
]

export default async function VoorRetailPage({ params }: Props) {
  const { locale } = await params
  setRequestLocale(locale)
  const t = await getTranslations('voorRetail')
