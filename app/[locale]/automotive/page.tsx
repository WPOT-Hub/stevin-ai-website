...
const featureIcons = [
  <Search key="1" className="w-5 h-5 text-accent" />,
  <MapPin key="2" className="w-5 h-5 text-accent" />,
  <ShieldAlert key="3" className="w-5 h-5 text-accent" />,
  <Radio key="4" className="w-5 h-5 text-accent" />,
  <Calendar key="5" className="w-5 h-5 text-accent" />,
  <Radar key="6" className="w-5 h-5 text-accent" />,
]

export default async function AutomotivePage({ params }: Props) {
  const { locale } = await params
  setRequestLocale(locale)
  const t = await getTranslations('automotive')
