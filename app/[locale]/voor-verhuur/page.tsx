...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'verhuur' })
  return localizedMetadata({ path: '/voor-verhuur', locale, title: `Stevin ${t('eyebrow')}, ${t('h1')}`, description: t('sub') })
}

const featureIcons = [
  <Boxes key="1" className="w-5 h-5 text-accent" />,
  <MapPin key="2" className="w-5 h-5 text-accent" />,
  <CalendarClock key="3" className="w-5 h-5 text-accent" />,
  <Plug key="4" className="w-5 h-5 text-accent" />,
  <Activity key="5" className="w-5 h-5 text-accent" />,
  <Radar key="6" className="w-5 h-5 text-accent" />,
]

export default async function VerhuurPage({ params }: Props) {
  const { locale } = await params
