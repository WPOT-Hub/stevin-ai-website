...
import { MapPin, RefreshCw, Wallet, FlaskConical, GitBranch, Radar } from 'lucide-react'

type Props = { params: Promise<{ locale: string }> }

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'autodealerCampagnes' })
  return {
...
const featureIcons = [
  ...
  <RefreshCw key="2" className="w-5 h-5 text-accent" />,
  <Wallet key="3" className="w-5 h-5 text-accent" />,
  <FlaskConical key="4" className="w-5 h-5 text-accent" />,
  <GitBranch key="5" className="w-5 h-5 text-accent" />,
  <Radar key="6" className="w-5 h-5 text-accent" />,
]

export default async function AutodealerCampagnesPage({ params }: Props) {
  const { locale } = await params
  setRequestLocale(locale)
  const t = await getTranslations('autodealerCampagnes')
